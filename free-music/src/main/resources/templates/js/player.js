...
function play(music) {
    ...
    if(Player.debug) {
        ...
        'source: "' + music.source + '",\n' +
        'pic_id: "' + music.picId + '",\n' +
        'pic_url: "' + music.picUrl + '",\n' +
        'url: "' + music.musicUrl + '",\n' +
        'lyric: "' + music.lyricId + '",\n');
    }
    
    // 遇到错误播放下一首歌
    if(music.musicUrl == "err") {
        audioErr(); // 调用错误处理函数
        return false;
    }
    ...
    try {
        rem.audio[0].pause();
        rem.audio.attr('src', music.musicUrl);
        rem.audio[0].play();
    } catch(e) {
        audioErr(); // 调用错误处理函数
        return;
...
var initProgress = function(){  
    ...
    tmp_vol = (tmp_vol != null)? tmp_vol: (rem.isMobile ? 1: Player.volume);
