...
$(function(){
    ...
    $(".btn-quiet").click(function(){
        ...
        if($(this).is('.btn-state-quiet')) {
            ...
            oldVol = oldVol? oldVol: (rem.isMobile? 1: Player.volume);  // 没找到记录的音量，则重置为默认音量
    ...
    if((Player.coverBg === true && !rem.isMobile) || rem.isMobile) { // 开启了封面背景
...
function changeCover(music) {
    ...
    if(img == "err") {
        ...
        // 移动端封面
        if(Player.coverBg === true && rem.isMobile) {
            $("#music-cover").load(function(){
                $("#mobile-blur").css('background-image', 'url("' + img + '")');
            });
        } else if(Player.coverBg === true && !rem.isMobile) { // PC端封面
            $("#music-cover").load(function(){
                if(animate) {   // 渐变动画也已完成
    ...
    $.ajax({
        ...
        success:function (data) {
            ...
            for(let i = 1; i < musicList.length; i++) {
                if(i === 1) {    // 正在播放列表
                    // 读取正在播放列表
                    let tmp_item = playerReaddata('playing');
                    if(tmp_item) {  // 读取到了正在播放列表
                        musicList[1].item = tmp_item;
                        Player.defaultlist = 1;   // 默认显示正在播放列表
                    }
                } else if(i === 2) { // 历史记录列表
                    // 读取历史记录
            ...
            // 首页歌单加载默认歌单
            ajaxPlayList(musicList[Player.defaultlist].id, Player.defaultlist);
            // 默认列表加载完成后,展示到首页
